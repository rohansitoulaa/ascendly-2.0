...
const DotField = memo(function DotField({
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      // Idle exit: if mouse has been still > 600ms AND the field is essentially at rest,
      // stop requesting frames. mousemove / visibilitychange will restart.
      const idleMs = performance.now() - lastMouseMoveTs;
      const atRest =
        engagement.current < 0.002 &&
        glowOpacity.current < 0.004 &&
        m.speed < 0.01;
      if (idleMs > 600 && atRest) {
        isRunning = false;
        rafRef.current = null;
        return;
      }
